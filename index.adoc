...
This will follow the rapid application development by iteratively adding features from user stories. The first part will
be creating the objects and controllers. From there I will gather more user stories for specific parts of the application.

=== Variable naming
Variable names will be clear from context and names. Single letter variables will be used sparingly, mostly as iterators
and occasionally in lambda functions.

=== Success Criteria
What needs to be done.

* Supports adding League
* Supports adding Teams
* Supports adding Players
* Supports adding Games
* Supports adding Players to Games
* Supports adding Scores to Players in Games
* Shows all the Leagues, Teams, Players, Games and Scores
* Shows High Game on Player Level
* Shows Low Game on Player Level
* Shows High Series on Player Level
* Shows Low Series on Player Level
* Shows Recent Games on Player Level
* Shows HHG on Team level
* Shows HHS on Team level
* Shows Pins For on Team level
* Shows Pins Against on Team level
* Shows Games Played on Team level
* Shows a comparison of all team stats on League Level
* Shows upcoming games at league level
* Requires users to be authenticated by OAuth 2
* Supports calculating and using a handicap for a player
* Supports scoring a game and adding totals
* Supports setting blind scores in a Game
* Supports creating a rota of games from length and availability
* Supports elimination tournaments
...
These were necessary as it gives a place to store all the objects. Anything here goes through hibernate to
communicate with the H2 database with SQL.
...
==== HomeController.java
This will route to the javascript and html to sort out the front end
...
public class HomeController {
    ...
    public String index(){
        return "index";
        // Show index file extension guessed, currently .html
    }
}
----
...
js into one bundle.js file to be sourced from the template (index.html). By editing a basic one I found in docs I made
[source,js]
----
...
This will provide places for users to add and remove leagues.
...
This is a similar controller supporting adding and removing teams.
...
This will support addition of player to Teams and deletion of players from Teams.
...
Once again, a name and an id for identification and redirects.
...
=== Player Front End
Gives users the ability to create and remove players from the front end.
...
This will allow games to be created and removed.
...
This will make it available for users to create and remove games from the front end.
...
=== Functionality
The program in it's current state provides an user friendly interface to add leagues, teams, players, games and scores.
It has features additionally which generates statistics on players based on the games. It is missing features such as blind score
and some options for tournaments. It also includes input checking to reduce crashes and invalid data.

==== What it should do, and what it does
...
==== Usability
The program has many aspects to make it easy for a client to use. Especially in the polish step, many parts were added
to provide continuity in the user interface. These include a consistent back button in the top right of the page,
a consistently placed title header at the top with crucial detail, and use of the same type of buttons for the same tasks
(back white, submit/add blue). Alongside the tables and lists to nicely display all the data it provides a functional
and easy to use interface for the client and anyone else using it.

=== My thoughts
I don't think the program is complete, but I think it offers a nicer interface to use for the league. However due to a
lack of some features it is not currently fit to be a full replacement of the current system. Despite this I think
that the solution functions well and is very stable as shown by the testing.


=== Client thoughts
The client agrees that some crucial features are missing before it could be implemented as a replacement, however the
features present are far easier to use than uploading all the HTML every time anything changes.
In particular the client likes the interface as a time saving measure to display near instantly updated statistics as
soon as the scores are added. He also likes the options to save a lot of effort in modifying the existing excel spreadsheet
by instead using the UI or being able to allow other people to use the UI, so he can have a break.
The client is very interested in adding blind scores and similar features so it can replace the existing system entirely.
...
This will allow the project to get new features. An example is someone could add an issue for blind scores.
Add the function to `Game.js` to show it in front end. Add it to the `Player` object, and submit a pull request.
Then I can review the pull request and merge it to add the feature.

=== Maintenance
The code should stay functioning with the correct versions of the libraries. For deployment it should be
put into a docker container on a server. The database should then be frequently backed up in order to prevent data loss.
It includes commented code and a readme to make it easy for future contributors to contribute to the project without
requiring a lot of time to learn how it works.
Due to the current implementation however it may be harder to add many features without making controllers a lot longer
and more cryptic than the current state.
...
- [[[MyRepo]]] Callum Tarttelin. The repository I used for version control, full code is available here : github.com/saskcow/bowling . Accessed April 2018
